...
| D35 (2026-07-31, Phase 11F) | **The desktop rail collapses**, and its state is a **cookie read server-side** — `--rail-width` is the only thing the toggle changes, so the content column and the docked session bar follow it for free. Labels stay in the accessibility tree when collapsed. Two deliberate `docs/08 §4` exceptions: the rail's `width` and the content column's `padding-left` are animated. **Home scrolls normally on desktop** — an earlier attempt to lock it to the viewport with per-panel internal scrolling was built, tested, and reverted on review. | The cookie beats `localStorage` because the server can then stamp the width into the first paint; the usual `localStorage` fix is an inline `<head>` script, which would saddle Phase 10's CSP with `script-src 'unsafe-inline'` for a UI preference. A resizing rail has no `transform` equivalent — it is one user-initiated layout change, not decoration — so it animates, and is disabled under reduced motion. On the viewport-lock: it *did* fit, but only by squeezing the panel row to a few hundred pixels and moving the scroll inside each card, which is a worse reading than one honest page scroll — **Records in particular wants full width and room, not a scrollport**. | `localStorage` + an inline bootstrap script (CSP debt), snapping the rail with no transition (a 136px jump reads as a glitch), keeping the fill layout (nested scroll regions, cramped panels) |
| D36 (2026-08-06, Phase 11N) | **A plan is a separate table, and a future-dated session is not in progress.** Prescribed work lives in `planned_sets` — never a flag on `exercise_sets` — so no analytics or PR query can see it; `plans.complete` is the one door across, and it writes through `sets.log_set` rather than a second write path. Completion is **derived** (`completed_set_id` + "is that set still live?"), off-plan logging is never blocked, and `finish_session` reports adherence with `percent = null` when nothing was prescribed. Separately, `get_active_session` now **skips** any open session starting more than `SCHEDULING_SKEW` (5 min) ahead of now | An `is_planned` flag would make six aggregate queries responsible for the invariant, and the first one that forgot would credit a lifter with work they were only told to do — a separate table holds it by construction, with nothing to remember. Deriving completion is what makes `delete_set` reopen a prescribed line with no second write and no way for the two to disagree. The skew guard fixes a live hazard planning created: both staleness clauses in `get_active_session` measure a *negative* age for a future date, so a plan for next Tuesday passed every "still live" test and, ordered newest-first, sorted above the workout actually under way and took its place for days | An `is_planned` column on `exercise_sets` (every aggregate becomes a place to forget), storing `is_completed` as a boolean (drifts the moment a set is deleted or restored), refusing sets that no line prescribes (a tool that blocks training because nobody wrote it down first is worse than no plan), a `status` column on `workout_sessions` (re-litigates D31's `ended_at` lifecycle to say something the data already answers), reporting 0% adherence for an unplanned session (a judgement about a plan that never existed) |
| D37 (2026-08-06, Phase 11N) | **`analytics.frequency` counts sessions you trained, not sessions that exist**: a session holding a prescription and **no live set** is excluded, and both aggregates now filter `deleted_at`. An *unplanned* empty session still counts | `frequency` counts `workout_sessions` rows rather than `exercise_sets`, so it is the one arm of "a plan is never training" that a separate table cannot hold by construction — planning Mon/Wed/Fri a week ahead reported three sessions trained while volume was `[]` and there were no records. The soft-delete filter is a pre-existing gap on the same line of code: deleting a session left its tonnage in every total permanently, and `purge_deleted` frees storage rather than undoing a number. Keeping the unplanned-empty case counting preserves what "I started a workout" has meant since Phase 2 — planning is not a reason to redefine it | Excluding every session with no sets (silently changes an existing meaning, and an empty session *is* a session you started), a `status` column to filter on (see D36), leaving `frequency` alone and documenting the caveat (the guide shipped to MCP clients states the opposite, and a wrong number nobody can see is still wrong) |
